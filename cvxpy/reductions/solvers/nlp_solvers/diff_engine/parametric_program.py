...
def _lazy_tensor(attr: str) -> property:
    """A ParamConeProg tensor attribute that re-encodes on first read."""
    def getter(self):
...
class DiffengineParamConeProg(ParamConeProg):
    ...
    # Re-encoded lazily: solvers read apply_parameters' return values, so
    # doing it eagerly costs ~2x the re-extraction for data nothing reads.
    q = _lazy_tensor('_q_tensor')
    A = _lazy_tensor('_A_tensor')
    P = _lazy_tensor('_P_tensor')
    reduced_A = _lazy_tensor('_reduced_A')
    reduced_P = _lazy_tensor('_reduced_P')

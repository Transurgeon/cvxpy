...
def convert_quad_form(expr, children):
    ...
    A constant ``P`` is routed by ``make_constant_quad_form`` (shared with the
    DCP-path SymbolicQuadForm converter). A parametric ``P`` (depends on
    parameters, not on ``x``) is fed to the dense path as the matrix-valued
    child ``children[1]``, which the engine re-evaluates each solve.
    """
    P = expr.args[1]
    n = expr.args[0].size

    ...
    def format_constraints(cls, problem, exp_cone_order):
        ...
        if signs is not None:
            ...
            if (signs > 0).all():
                ...
                m = signs.shape[0]
                # problem.A is the parameter tensor: its row i + m*j holds
                # concrete constraint row i of variable block j. So scaling
                # every stored value by the sign of its own concrete row
                # applies diag(signs) to all n+1 blocks in one pass.
                # CSC is required (not just faster): only there does `indices`
                # hold row indices. copy=True because .data is rebound below.
                A = problem.A.tocsc(copy=True)
                concrete_row = A.indices % m
                A.data = A.data * signs[concrete_row]
                restructured_A = A
        elif restruct_mat:

...
class ConeFormat(Reduction):
    ...
    re-extractable form (the DIFFENGINE backend's) restructure itself instead
    of being rebuilt as a stock program, via ``ParamConeProg.format_for``.

    A no-op when there is nothing to do: an already-formatted program, or a
    program whose solver formats nothing.
    """
